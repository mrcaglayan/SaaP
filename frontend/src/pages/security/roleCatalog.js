...
const WORKFLOW_AUTHORITY_CATALOG = Object.freeze({
  ...
  CLOSE_CHECKLIST_TASK: freezeList([
    freezeAuthority({
      code: "CLOSE_TASK_VIEW",
      displayName: "View close checklist tasks",
      description: "Read close checklist tasks, template catalog rows, and task audit events.",
      anyPermissionCodes: ["close.task.read", "close.task.template.read"],
      allowedStepScopes: ["OPERATING_UNIT", "LEGAL_ENTITY", "COUNTRY", "GROUP"],
      viewOnly: true,
      sortOrder: 10,
    }),
    freezeAuthority({
      code: "CLOSE_TASK_PREPARE",
      displayName: "Prepare close checklist tasks",
      description: "Create manual tasks and work assigned close checklist tasks.",
      requiredPermissionCodes: ["close.task.work"],
      anyPermissionCodes: ["close.task.create"],
      allowedStepScopes: ["OPERATING_UNIT", "LEGAL_ENTITY", "COUNTRY"],
      sortOrder: 20,
    }),
    freezeAuthority({
      code: "CLOSE_TASK_REVIEW",
      displayName: "Review close checklist tasks",
      description: "Return or approve submitted close checklist tasks.",
      requiredPermissionCodes: ["close.task.review"],
      allowedStepScopes: ["LEGAL_ENTITY", "COUNTRY", "GROUP"],
      sortOrder: 30,
    }),
    freezeAuthority({
      code: "CLOSE_TASK_WAIVE",
      displayName: "Waive close checklist tasks",
      description: "Waive close checklist tasks with a mandatory governance reason.",
      requiredPermissionCodes: ["close.task.waive"],
      allowedStepScopes: ["COUNTRY", "GROUP"],
      sortOrder: 40,
    }),
    freezeAuthority({
      code: "CLOSE_TASK_ADMIN",
      displayName: "Administer close checklist tasks",
      description: "Manage assignment, lifecycle overrides, and task template governance.",
      anyPermissionCodes: ["close.task.assign", "close.task.admin", "close.task.template.write"],
      allowedStepScopes: ["OPERATING_UNIT", "LEGAL_ENTITY", "COUNTRY", "GROUP"],
      sortOrder: 50,
    }),
  ]),
