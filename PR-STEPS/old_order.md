...
1. PR-F01: Platform prerequisites and feature flags
2. PR-F02: Subaccounts schema hardening (`m081`) + bank OU ownership
3. PR-F03: `102` subtree enforcement + post-usage immutability
4. PR-F05: Setup Wizard V2 + onboarding account-tree payload
5. PR-F06: Country pack expansion + onboarding binding
6. PR-F07: Workflow approval engine foundation (`m082`)
7. PR-F08: Close/consolidation staged approval gating
8. PR-F10: Country tax engine foundation (`m083`)
9. PR-F11: Tax runtime engine + CARI integration
10. PR-F04: One-click bank auto-provision (`102` child + bank account)
11. PR-F09: Workflow UI + readiness wiring
12. PR-F12: Canonical consolidation mapping convergence wiring
13. PR-F13: Backfill + rollout + release-gate expansion
...
- [ ] PR-F07 acceptance: `m082` workflow approval schema and read/setup APIs are available for `PERIOD_CLOSE` and `CONSOLIDATION_RUN`, with scoped assignment resolution and validation.
