...
|---|---|---|
| `NONE` | Current default — manual account selection, plain GL posting | Today's default |
| `STOCK` | References an inventory card, account auto-resolved from product/warehouse config | Today's `stock_impact_mode != 'NONE'` pattern |
| `FIXED_ASSET` | References a draft (purchase) or active (sale) fixed asset, account auto-resolved from asset category | NEW |
...
- Line 3: CNC Machine → `FIXED_ASSET` → draft asset reference

### Purchase Flow (AP + FIXED_ASSET)

1. User creates a **DRAFT fixed asset** first (name, category, OU, depreciation settings) — this already exists via the manual create form
2. User creates a **CARI AP document**
3. On a line, user selects `subledger_type = FIXED_ASSET`
4. User picks the **draft asset** from a dropdown/search
5. Posting account **auto-resolves** to the asset category's asset account (no manual account selection needed)
6. User **posts the bill**
7. Posting logic sees FIXED_ASSET line → creates a CAPITALIZATION transaction on the referenced draft asset, updates cost fields (`original_cost_txn`, `original_cost_base`, `acquisition_date`)
8. User goes to asset detail → **activates** (depreciation schedule generated, status → ACTIVE, no additional journal needed — cost already posted via CARI)
...
3. User picks the **ACTIVE asset** being sold
...
|---|---|
| Post bill → go to FA module → capitalize from AP line → create asset after the fact | Create draft asset → enter bill with FA line → post bill → asset gets cost automatically |
...
| Asset name derived from bill line description | Asset named by user when creating the draft, bill line references it |
| `postingAccountId` manually picked | Account auto-resolved from asset category |

### Inline Asset Creation from the Bill (Pattern A)

Real ERP systems handle the "asset doesn't exist yet" case in three ways:

| Pattern | Used by | How it works |
|---|---|---|
| **A: Inline Create (Modal)** | SAP, Dynamics, modern SaaS | "+" button on the bill line opens a dialog → create asset → auto-select on line. User never leaves the bill. |
| **B: Auto-Create from Category** | SAP classic | Select only a category on the line → system auto-creates asset on posting. Less control. |
...
This plan uses **Pattern A** — a lightweight inline create modal on the asset picker. When the user is entering a bill and the draft asset doesn't exist yet:

1. Select `FIXED_ASSET` on the line
2. Open the asset picker → click **"+ New Asset"**
3. Modal shows: name, category, owner OU, location OU (category auto-fills depreciation defaults)
4. Save → draft created → auto-selected on the line
5. Continue entering the bill

This is the standard UX in modern SaaS ERP. The full asset detail (serial number, legacy onboarding, account overrides) can be edited on the asset detail page after creation — the modal only collects the essentials.
...
```
settlement_mode  ENUM('ACCRUAL', 'IMMEDIATE_CASH', 'IMMEDIATE_BANK')  DEFAULT 'ACCRUAL'
```
...
| `IMMEDIATE_BANK` | On posting: bill posted + bank payment auto-created + CARI settlement auto-applied. Requires `settlement_bank_account_id`. |
...
- [ ] `STEP-SL01` — Migration: add `subledger_type` and `target_fixed_asset_id` to `cari_document_lines`
...
- [ ] `STEP-SL05` — 🔥 HOT — Backend CARI posting: FIXED_ASSET AP line → create CAPITALIZATION transaction on target draft asset
- [ ] `STEP-SL06` — 🔥 HOT — Backend CARI posting: FIXED_ASSET AR line → trigger disposal flow on target active asset
...
- [ ] `STEP-SL11` — Frontend CARI form: FIXED_ASSET conditional fields (asset picker, inline create, auto-account display)
...
- [ ] `STEP-SL15` — Migration: add `settlement_mode`, `settlement_cash_register_id`, `settlement_bank_account_id` to `cari_documents`
- [ ] `STEP-SL16` — 🔥 HOT — Backend CARI posting: immediate settlement (auto-create cash/bank payout + auto-apply CARI settlement)
- [ ] `STEP-SL17` — Frontend CARI form: settlement mode selector (Accrual / Cash Purchase / Bank Payment)
...
- Add CHECK constraint: `CHECK (subledger_type != 'FIXED_ASSET' OR target_fixed_asset_id IS NOT NULL)` — prevents persisting a FIXED_ASSET line without a target asset reference
...
### Patch target
Extend the CARI document line validator to accept and validate `subledgerType` and `targetFixedAssetId`.

### In scope
- Parse `subledgerType` from line payload
- **Backward-compat inference rule** (CRITICAL): When `subledgerType` is omitted or absent:
  - If `itemCardId` is present OR `stockImpactMode != 'NONE'` → auto-infer `subledgerType = 'STOCK'`
  - If `targetFixedAssetId` is present → auto-infer `subledgerType = 'FIXED_ASSET'`
  - Otherwise → default `'NONE'`
  - **Why**: Existing clients send `{ itemCardId, stockImpactMode, warehouseId }` without any `subledgerType` field. Without this inference rule, those requests would be treated as `NONE` and the stock fields would be rejected or persisted with the wrong subledger_type. The existing validator at `cari.document.validators.js:254` already accepts these fields without subledgerType.
- When `subledgerType = 'FIXED_ASSET'` (explicit or inferred): require `targetFixedAssetId`, reject `itemCardId` and `stockImpactMode != 'NONE'`
- When `subledgerType = 'STOCK'` (explicit or inferred): require `itemCardId` and `stockImpactMode != 'NONE'`, reject `targetFixedAssetId`
- When `subledgerType = 'NONE'`: reject `targetFixedAssetId`, allow normal fields (including `itemCardId = null` which is already the case)
...
### Definition of done
- Existing create/update document requests without subledgerType continue to work unchanged (inference rule auto-promotes to STOCK when stock fields present)
- subledgerType=FIXED_ASSET without targetFixedAssetId returns 400
- subledgerType=FIXED_ASSET with itemCardId returns 400
- subledgerType=STOCK without itemCardId returns 400
- Legacy request `{ itemCardId: 5, stockImpactMode: 'IN', warehouseId: 3 }` (no subledgerType) succeeds and persists `subledger_type = 'STOCK'`
...
### Patch target
Persist `subledger_type` and `target_fixed_asset_id` when creating or updating CARI document lines.

### In scope
- **Update `replaceDocumentLinesTx`** (currently at line ~2497 in `cari.document.service.js`): add `subledger_type` and `target_fixed_asset_id` to the hardcoded INSERT statement (currently 19 columns — becomes 21)
- **Update line normalization** (currently at line ~2128): parse and pass through `subledgerType` and `targetFixedAssetId` from the validated input
- **Update `loadDocumentLinesForDocument`** SELECT queries (around line ~775-855): add `subledger_type` and `target_fixed_asset_id` to the SELECT list and include them in the response mapping
- When `subledger_type = 'FIXED_ASSET'`: validate that `target_fixed_asset_id` references an existing asset in the same tenant/legal entity
- For AP documents: validate target asset is in `DRAFT` status (asset must exist before the bill)
...
- Return `subledgerType` and `targetFixedAssetId` in document line read responses
...
### Definition of done
- Creating a CARI AP document with a FIXED_ASSET line and valid draft asset ID succeeds
...
- Document read responses include the new fields
- `replaceDocumentLinesTx` persists `subledger_type` and `target_fixed_asset_id` correctly
...
### In scope
- For AP + FIXED_ASSET: resolve `posting_account_id` to the target asset's `default_asset_account_id` (from its category)
...
### Definition of done
- FIXED_ASSET line on AP draft auto-resolves to asset account
- FIXED_ASSET line on AR draft auto-resolves to disposal gain account
...
## `STEP-SL05` — Backend CARI posting: FIXED_ASSET AP line → create CAPITALIZATION transaction on target draft asset

### Patch target
When posting a CARI AP document, lines with `subledger_type = 'FIXED_ASSET'` trigger the fixed-asset capitalization flow.
...
- During `postCariDocumentByIdTx()`, **after** the CARI journal is created via `insertPostedJournalWithLinesTx` (and after stock links at line ~4293), add a new `processFixedAssetLineSideEffectsTx()` call — analogous to the existing `replaceDocumentLineStockLinksTx` pattern. For each FIXED_ASSET line:
  - Re-validate target asset is still DRAFT and in the same tenant/legal entity (SELECT ... FOR UPDATE to lock the asset row)
  - Create a CAPITALIZATION `fixed_asset_transaction` row with these field values:
    ...
    - `journalEntryId`: the CARI posting journal's `journal_entry_id` (the one just created — **do NOT create a separate journal**)
    - `sourceRefType`: `'CARI_DOCUMENT'`
    - `sourceRefId`: document ID
    - `sourceRefLineId`: line ID
    - `grossAmountTxn/Base`: line net amounts
    - `accumDeprAmountTxn/Base`: 0 (new asset)
    - `nbvAmountTxn/Base`: = gross amount (new asset)
  ...
  - Update the asset's cost fields: `original_cost_txn`, `original_cost_base`, `acquisition_date` (from document date), `currency_code`, `source_cari_document_id`, `source_cari_document_line_id`, **`source_cari_document_line_unit_no = 1`** (see below)
- **Unit number convention**: Set `source_cari_document_line_unit_no = 1` for all SL05-created capitalizations. The existing FA06 flow uses unit slots (1, 2, 3...) for multi-asset splitting from a single line. The SL05 flow is always 1:1 (one CARI line = one draft asset), so unit_no = 1 is the correct sentinel. This satisfies the `uk_fa_cari_provenance` unique key and the `revalidateSourceLinkageForActivation` function which requires all three source columns to be non-null (see SL08 for the dual-mode revalidation).
- If target asset is no longer DRAFT at posting time, reject the posting with a clear error

### Explicit non-goals
- Do not change posting logic for NONE or STOCK lines
- Do not change the activation flow (SL08 handles that)
- Do not create the asset — it must exist as DRAFT before the bill is entered
- Do not create a separate journal — the CARI posting journal already has the correct debit/credit lines

### Definition of done
- Posting an AP doc with a FIXED_ASSET line creates a CAPITALIZATION transaction on the draft asset
- The CAPITALIZATION transaction references the CARI posting journal (same `journal_entry_id`, no duplicate journal)
- Asset cost fields are updated from the line amounts
- `source_cari_document_line_unit_no` is set to `1`
...
- Posting is rejected if target asset is not DRAFT

---

## `STEP-SL06` — Backend CARI posting: FIXED_ASSET AR line → trigger disposal flow on target active asset
...
### Explicit non-goals
- Do not remove the existing multi-step sale staging API (FA39) — it remains as an alternative path
...
- For AP reversal with FIXED_ASSET lines:
  - Find the CAPITALIZATION transaction on `fixed_asset_transactions` where `source_ref_type = 'CARI_DOCUMENT'`, `source_ref_id = documentId`, `source_ref_line_id = lineId`, `reversal_transaction_id IS NULL`
  - If not found (already reversed independently), skip this line's FA side-effects
  - If found AND asset status != DRAFT, **block the CARI reversal** with error: "Asset {assetNo} has been activated since capitalization. Reverse the activation first before reversing the source CARI document."
  - If found AND asset is still DRAFT: reverse the CAPITALIZATION transaction, restore cost fields to zero, clear `source_cari_document_id/line_id/unit_no`
- For AR reversal with FIXED_ASSET lines:
  ...
  - Reverse the SALE transaction + cutoff depreciation transaction
...
### Definition of done
- Reversal document lines include `subledger_type` and `target_fixed_asset_id` (copied from original)
- Reversing an AP doc with FIXED_ASSET lines restores the draft asset to pre-capitalization state
...
The new SL05 flow sets all three columns (with `unit_no = 1`), so the existing function WILL find non-null values. However, the FA06 flow assumes that `quantity > 1` is possible and does equal-split math, while the SL05 flow is always 1:1. The revalidation must detect which path created the asset and behave accordingly:

- **Detection**: Check if a CAPITALIZATION transaction exists on `fixed_asset_transactions` with `source_ref_type = 'CARI_DOCUMENT'` for this asset. If yes → SL05 path. If no → FA06 path (the FA06 flow creates the asset AND the transaction at the same time, so by activation time the transaction always exists — but it was created by `createAssetsFromCariDocumentLineFa06`, not by CARI posting). **Alternative simpler detection**: since SL05 always sets `unit_no = 1` and the asset is created BEFORE the bill, we can distinguish by checking if the asset was created via the new flow. The cleanest approach: add a boolean column `is_cari_posting_capitalized` to `fixed_assets` (or a flag field), set to `true` by SL05 when it capitalizes. Or simply: if the asset already has a CAPITALIZATION transaction with a `journal_entry_id` that belongs to a CARI posting journal, it's the SL05 path.
...
- **SL05 path (CARI-capitalized asset)**:
  ...
  - Validate asset cost matches the line amount (no per-unit split needed — it's 1:1)
  - Skip creating a new capitalization journal (cost already journaled via CARI posting)
  - Generate depreciation schedule as normal
  - Change status to ACTIVE
  - The activation becomes a metadata operation, not an accounting event
...
- Asset without any CAPITALIZATION (manual path) activates with the normal ACQUISITION journal
...
- When changed, clear fields that belong to the previous type (e.g. switching from STOCK to FIXED_ASSET clears itemCardId, switching from FIXED_ASSET to NONE clears targetFixedAssetId)
...
- Default is NONE
- Existing document creation continues to work (NONE = current behavior)

---

## `STEP-SL11` — Frontend CARI form: FIXED_ASSET conditional fields (asset picker, auto-account display)

### Patch target
When `subledgerType = 'FIXED_ASSET'` is selected on a line, show the asset picker and auto-resolved account.

### In scope
- Show a **draft asset picker** (search/select from existing DRAFT assets) for AP documents
- Show an **active asset picker** (search/select from existing ACTIVE assets) for AR documents
- Asset picker shows: asset no, name, category, owner OU
- When an asset is selected, display the auto-resolved account (read-only, from category)
- Hide the manual `postingAccountId` field when subledger_type is FIXED_ASSET
- When `subledgerType = 'STOCK'`: show existing `itemCardId`, `warehouseId`, `stockImpactMode` fields (current behavior)
- When `subledgerType = 'NONE'`: show existing manual fields (current behavior)
- **Inline "Create Asset" shortcut** (Pattern A — standard in SAP, Dynamics, modern SaaS):
  - Inside the asset picker, show a **"+ New Asset"** button when no match is found or when the user wants to create one on the spot
  ...
  - For AR documents (sale), the "Create" shortcut is hidden (you can only sell existing active assets)
...
- Do not auto-create the asset on bill posting (Pattern B) — the user must explicitly create and select

### Definition of done
- FIXED_ASSET line shows asset picker appropriate to document direction (draft for AP, active for AR)
- Selected asset shows auto-resolved account (read-only)
- AP document: asset picker includes a "+ New Asset" button that opens a quick-create modal
- Modal creates a draft asset and auto-selects it on the line without navigating away
- AR document: no create shortcut (sale only references existing active assets)
- STOCK line shows existing inventory fields
- NONE line shows existing manual fields
...
### Patch target
Enforce form-level validation rules based on the selected subledger type.

### In scope
- FIXED_ASSET: `targetFixedAssetId` required, `itemCardId` must be empty, `postingAccountId` auto-resolved (read-only)
- STOCK: `itemCardId` required, `stockImpactMode` required, `targetFixedAssetId` must be empty
- NONE: standard existing validation
...
### Definition of done
- Cannot submit a FIXED_ASSET line without selecting an asset
...
- Add a note: "To capitalize from a vendor bill, first create a draft asset, then enter the bill with a Fixed Asset line type"
...
### Patch target
Add the schema foundation for immediate cash/bank settlement on CARI documents.

### In scope
- Add `settlement_mode` ENUM('ACCRUAL', 'IMMEDIATE_CASH', 'IMMEDIATE_BANK') NOT NULL DEFAULT 'ACCRUAL' to `cari_documents`
- Add `settlement_cash_register_id` BIGINT UNSIGNED NULL to `cari_documents` — FK to `cash_registers(id)`
- Add `settlement_bank_account_id` BIGINT UNSIGNED NULL to `cari_documents` — FK to `bank_accounts(id)`
...
- Add `auto_settlement_cash_transaction_id` BIGINT UNSIGNED NULL to `cari_documents` — tracks the cash/bank transaction created by immediate settlement for reversal lookup
- Add CHECK-like constraint: `settlement_cash_register_id` must be set when `settlement_mode = 'IMMEDIATE_CASH'`, `settlement_bank_account_id` must be set when `settlement_mode = 'IMMEDIATE_BANK'`
...
## `STEP-SL16` — Backend CARI posting: immediate settlement (auto-create cash/bank payout + auto-apply CARI settlement)

### Patch target
When posting a CARI document with `settlement_mode = 'IMMEDIATE_CASH'` or `'IMMEDIATE_BANK'`, automatically create the payment transaction and apply the CARI settlement in the same database transaction.
...
- During `postCariDocumentByIdTx()`, after the normal AP/AR journal is posted and open item is created:
  ...
  - If `settlement_mode = 'IMMEDIATE_BANK'`:
    - Create a bank payment entry via bank transaction `*Tx` variant on the specified `settlement_bank_account_id`
    - Auto-apply CARI settlement via `applyCariSettlementTx(tx, ...)`
    - Store created IDs on the document
  - Both modes: mark the document as immediately settled (the open item balance should be zero after posting)
- Validate that the cash register / bank account belongs to the same tenant and legal entity
...
- `settlement_mode = 'IMMEDIATE_BANK'` works analogously with bank payment
- `settlement_mode = 'ACCRUAL'` (default) works exactly as before — no change
- Cash register / bank account validation rejects wrong tenant or missing session
...
- Add a **"Payment"** section to the document header form:
  - Radio or dropdown: `On Credit (Accrual)` | `Cash Purchase` | `Bank Payment`
...
- When `Bank Payment` is selected: show a **bank account picker** (dropdown of bank accounts for the legal entity)
...
- Bank Payment shows bank account dropdown
- On Credit hides payment source fields
- Payload includes `settlementMode`, `settlementCashRegisterId` or `settlementBankAccountId`
...
- During CARI document reversal (within `reverseCariPostedDocumentById`), if the document has `settlement_mode != 'ACCRUAL'`:
  ...
  - Reverse the auto-created cash payout/receipt or bank payment via `reverseCashTransactionTx(tx, ...)`
...
- **Purchase smoke**: create draft asset → create AP doc with FIXED_ASSET line → post → verify asset has CAPITALIZATION transaction → activate → verify depreciation schedule generated
...
- **Reversal smoke**: post AP doc with FIXED_ASSET line → reverse → verify asset restored to DRAFT
...
- **Inline create smoke**: on CARI form, select FIXED_ASSET → use "+ New Asset" modal → verify draft created and auto-selected on line
...
- **Bank payment smoke**: create AP doc with `settlement_mode = IMMEDIATE_BANK` → post → verify bank payment created + settlement applied
...
- Verify OpenAPI spec includes new fields (subledger_type, target_fixed_asset_id, settlement_mode, settlement_cash_register_id, settlement_bank_account_id)
...
### `STEP-SL01`
- `AI size`: Small
- `Allowed files`: `backend/src/migrations/m142_cari_document_lines_subledger_type.js`, `backend/src/migrations/index.js`
...
### `STEP-SL09`
- `AI size`: Small
- `Allowed files`: `backend/src/migrations/m143_cari_document_lines_subledger_backfill.js`, `backend/src/migrations/index.js`
- `Dependencies`: SL01
- `Blocked by`: none
...
### `STEP-SL15`
- `AI size`: Small
- `Allowed files`: `backend/src/migrations/m144_cari_documents_settlement_mode.js`, `backend/src/migrations/index.js`
- `Dependencies`: none beyond repo baseline (cash_registers and bank_accounts tables must exist)
- `Blocked by`: none
- `Rollback risk`: Low
...
Phase 3 `IMMEDIATE_BANK` mode depends on:
