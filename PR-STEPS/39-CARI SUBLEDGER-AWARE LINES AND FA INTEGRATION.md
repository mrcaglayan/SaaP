...
- [x] `STEP-SL22` — i18n: AP/AR-specific labels (Vendor Bills, Sales Invoices, AP Payments, AR Receipts)
...
1. Implement `fixedAssetMode = 'IMPROVE_EXISTING'` posting for AP `FIXED_ASSET` lines:
   ...
   - link the posted CARI line to the target asset and improvement transaction for audit/drillback
2. Validate target asset state at draft-save and posting time:
   - target asset must exist in the same tenant/legal-entity scope
   - target asset must be eligible for improvement under locked policy
   - reject assets in statuses that are not allowed for improvement by the fixed-assets policy
   - reject `itemCardId` and any `stockImpactMode != 'NONE'`
3. Add a **depreciation-currentness gate** before allowing improvement:
   - block `IMPROVE_EXISTING` if the asset is not depreciation-current through the prior posted depreciation boundary
   - block when the current depreciation engine would still need to generate historical unposted rows such as `CATCH_UP`, sequence-gap repair, or equivalent missed-period recovery for that asset
   - return stable reason codes so the UI can show that improvement must wait until depreciation chronology is brought current
4. Add a **draft-run / frozen-snapshot gate**:
...
6. Support useful-life changes on improvement lines:
   ...
   - for `FULLY_DEPRECIATED` assets, require a life revision/extension that results in positive remaining life after posting
7. Recalculate remaining life using a rule aligned to the current depreciation engine, not a bespoke elapsed-month calculator:
   - base the recalculation on posted depreciation count / current useful-life semantics already used by depreciation services
   - update stored remaining-life fields consistently with existing fixed-asset depreciation behavior
8. Posting behavior for AP `FIXED_ASSET` lines with `fixedAssetMode = 'IMPROVE_EXISTING'`:
   - lock the target asset (`SELECT ... FOR UPDATE`)
   - re-validate target asset is still eligible at posting time
   - auto-resolve `posting_account_id` from the target asset category `default_asset_account_id`
   - reject explicit `postingAccountId`
   - snapshot pre-improvement state on the `IMPROVEMENT` transaction row:
     - pre cost in txn/base
...
9. Update API contract / OpenAPI for:
   ...
   - draft-run blocking reason codes
...
The existing FA Additions report (`fixed-assets.reporting.service.js`) filters `transaction_type IN ('ACQUISITION', 'CAPITALIZATION')` and does not include `IMPROVEMENT`. This remains intentional for Track 39 — improvements are cost augmentations on existing assets, not new asset additions. If a dedicated improvements report or expanded additions reporting is needed later, scope it as a follow-up reporting enhancement.
...
2. On reversal:
   ...
   - restore pre-improvement useful-life / remaining-life state from persisted transaction metadata
   - mark the improvement transaction reversed using the repo’s normal reversal/linkage style
3. Keep reversal blockers broader than only “later depreciation exists”:
   - block reversal if any later posted depreciation depends on the improved carrying state
   - block reversal if any later posted `IMPROVEMENT` exists on the same asset
   - block reversal if any later fixed-assets lifecycle activity exists whose accounting or chronology depends on the improved asset state, including ownership transfer, suspension/reactivation effects that depend on the post-improvement asset state, disposal/write-off/sale, or future retro-correction flows if they later land
4. Return stable machine-readable blocker codes so the UI and smoke scripts can distinguish:
   - later depreciation blocker
   - later improvement blocker
   - later lifecycle blocker
5. Keep reversal chronology-safe:
   - do not allow reversal that would leave later posted fixed-assets activity pointing to a state that no longer exists
   - do not silently recompute later posted depreciation or lifecycle rows during reversal
6. Keep the shared-journal rule explicit:
   - do not call the generic fixed-assets journal reversal on the shared CARI journal
   - the CARI document reversal remains the journal reversal driver
...
- Pre-state snapshot columns are used for restoration, not recomputation
- Shared-journal behavior is respected
...
3. Show enough target-asset context to confirm the correct selection:
   ...
   - category
   - current cost
   - current useful life / remaining life
   - current status
4. Show life-revision inputs with mutual exclusion:
   - `revisedUsefulLifeMonths`
   - `lifeExtensionMonths`
5. Surface backend chronology blockers clearly:
   - depreciation not current / catch-up pending
...
7. Keep generated-asset defaults hidden/disabled for this mode:
   ...
   - no owner/location generation fields required for improvement mode
   - auto-resolved account shown read-only from the target asset category
8. Update i18n labels and help text so `IMPROVE_EXISTING` is clearly distinguished from:
   - `AUTO_CREATE`
   - `LINK_EXISTING`
   - disposal/sale flows
...
- Do not auto-invalidate draft depreciation runs from the frontend
...
- Backend chronology/draft-run blockers are shown as actionable validation guidance
- The mode is clearly presented as prospective capitalization of an existing asset
- i18n labels exist in TR and EN
...
- No regression in existing fixed-assets flows
