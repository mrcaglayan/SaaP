...
async function resolveSmokeContext() {
  if (ENV_TENANT_ID && ENV_LEGAL_ENTITY_ID) {
    ...
    assert(
      legalEntityRows.rows?.[0],
      `legalEntityId ${ENV_LEGAL_ENTITY_ID} not found for tenant ${ENV_TENANT_ID}`
    );
    return {
      tenantId: ENV_TENANT_ID,
      legalEntityId: ENV_LEGAL_ENTITY_ID,
      currencyCode:
        String(legalEntityRows.rows[0].functional_currency_code || "").trim().toUpperCase() ||
        "USD",
    };
  }

  const preferredTenantResult = await query(
    `SELECT le.tenant_id,
      ...
      WHERE le.tenant_id = 1
        AND EXISTS (
              SELECT 1
                FROM users u
               WHERE u.tenant_id = le.tenant_id
                 AND u.status = 'ACTIVE'
            )
        AND EXISTS (
              SELECT 1
  ...
  const preferredTenantRow = preferredTenantResult.rows?.[0];
  if (preferredTenantRow) {
    return {
      tenantId: Number(preferredTenantRow.tenant_id),
      legalEntityId: Number(preferredTenantRow.legal_entity_id),
      currencyCode:
        String(preferredTenantRow.functional_currency_code || "").trim().toUpperCase() ||
        "USD",
    };
  }
  ...
  assert(
    row,
    "No smoke-ready legal entity found. Set FA24_SMOKE_TENANT_ID and FA24_SMOKE_LEGAL_ENTITY_ID if needed."
  );
