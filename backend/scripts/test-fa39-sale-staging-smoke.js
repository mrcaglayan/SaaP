  ...
  const r = await query(
    `SELECT le.tenant_id, le.id AS legal_entity_id
      ...
      WHERE le.tenant_id = 1
        AND EXISTS (SELECT 1 FROM roles r WHERE r.tenant_id=le.tenant_id AND r.code='TenantAdmin')
        AND EXISTS (
              SELECT 1 FROM accounts a JOIN charts_of_accounts c ON c.id=a.coa_id
              WHERE c.tenant_id=le.tenant_id AND c.legal_entity_id=le.id AND c.scope='LEGAL_ENTITY'
                AND a.account_type='ASSET' AND a.is_active=1 AND a.allow_posting=1)
        AND EXISTS (
              SELECT 1 FROM accounts a JOIN charts_of_accounts c ON c.id=a.coa_id
              WHERE c.tenant_id=le.tenant_id AND c.legal_entity_id=le.id AND c.scope='LEGAL_ENTITY'
                AND a.account_type='EXPENSE' AND a.is_active=1 AND a.allow_posting=1)
        AND EXISTS (
              SELECT 1 FROM accounts a JOIN charts_of_accounts c ON c.id=a.coa_id
              WHERE c.tenant_id=le.tenant_id AND c.legal_entity_id=le.id AND c.scope='LEGAL_ENTITY'
                AND a.account_type='REVENUE' AND a.is_active=1 AND a.allow_posting=1)
        AND EXISTS (
              SELECT 1 FROM books b JOIN fiscal_periods fp ON fp.calendar_id=b.calendar_id
              LEFT JOIN period_statuses ps ON ps.book_id=b.id AND ps.fiscal_period_id=fp.id
              WHERE b.tenant_id=le.tenant_id AND b.legal_entity_id=le.id AND b.book_type='LOCAL'
                AND fp.is_adjustment=0 AND COALESCE(ps.status,'OPEN')='OPEN')
      ORDER BY le.id ASC LIMIT 1`, []);
  const row = r.rows?.[0];
  assert(row, "No usable FA39 smoke context found");
  return { tenantId: Number(row.tenant_id), legalEntityId: Number(row.legal_entity_id) };
}
